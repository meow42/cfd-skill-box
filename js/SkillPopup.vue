<template>
  <div class="desc_wnd"> <!-- :style="`top: ${offset.top}px; left: ${offset.left}px`" -->
    ...
    <div class="dw_head">
      <div class="dw_icon">
        <img src="img/skillicon.png" class="skill_icon" :style="skill.getIconStyle()"/>
      ...
      <div class="dw_kind">
        ...
        {{ skill.kindTarget }}
        <br />
        {{ skill.needt ? `需要装备 ${skill.needt}` : '' }}
      </div>
    </div>
    <div class="dw_desc">
      <div v-show="skill.kind !== 'P'">{{ (skill.getN('ct') && skill.getN('ct') !== '无') ? 
                `施展时间 ${skill.getN("ct")}` : '立即释放' }}</div>
      <div v-show="skill.getN('dt')">冷却时间 {{ skill.getN("dt") }}</div>
      <div v-show="skill.getN('rg')">距离 {{ skill.getN("rg") }}</div>
      <div class="dw_blue">{{ skill.getN("nt") }}</div>
      <div class="dw_orange">{{ skill.text }}</div>
      <br>
      <div class="dw_disable">
        <span v-show="skill.getN('cl')" style="margin-right: 28px;">等级要求：{{ skill.getN("cl") }}</span>
        <span v-show="skill.needl">前置技能：Lv{{ skill.needl }}</span>
      </div>
    </div>
  </div>
</template>
